...
exitScope = modify (\s -> s { contexts = drop 1 (contexts s)})

emptyState :: Env
emptyState = Env Map.empty [] Type_void -- TODO
...
inferExp x = case x of
    ...
    ELt e1 e2           -> checkComparison e1 e2
    EGt e1 e2           -> checkComparison e1 e2
    ELtEq e1 e2         -> checkComparison e1 e2
    EGtEq e1 e2         -> checkComparison e1 e2
    EEq e1 e2           -> checkComparison e1 e2
    ENEq e1 e2          -> checkComparison e1 e2
...
checkComparison ::  Exp -> Exp -> StateError Type
checkComparison exp1 exp2 = do
   t1 <- inferExp exp1   
...
checkExp typ ex = do
    ...
    if typ2 == typ 
        then return typ
        else throwError $ "Wrong type of \"" ++ printTree ex ++ "\", expected " ++ printTree typ ++ " but found " ++ printTree typ2
